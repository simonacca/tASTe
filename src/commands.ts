...
const GrowShrinkAtSides = (
  ...
  if (side === "beginning" && action === "grow") {
    ...
    if (node?.previousNamedSibling) {
      return new vscode.Selection(sel.end, U.parserNode2Selection(node.previousNamedSibling).start)
    }
    ...
    return new vscode.Selection(sel.end, U.parserNode2Selection(node.nextNamedSibling).start)
    ...
    if (node?.nextNamedSibling) {
      return new vscode.Selection(sel.start, U.parserNode2Selection(node.nextNamedSibling).end)
    }
    ...
    return new vscode.Selection(sel.start, U.parserNode2Selection(node.previousNamedSibling).end)
  }
}
