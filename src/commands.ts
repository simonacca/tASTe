...
const setParserLanguageFromDoc = (
  ...
  if (!parser || !window.activeTextEditor) {
    return
  }
...
export async function ExpandSelection(
  ...
  const nodeToSelect = ASTpath.find(
...
export async function SelectNodeForward(
  ...
  const node = parseAndExtractLastNode(
    parser,
    doc,
    makeSelectionOfSize(
      movePositionChar(window.activeTextEditor.selection.end, -1),
      1,
    ),
  )
